...
function detectSubsystem(files) {
    ...
    for (const { file } of files) {
        ...
        else if (file.startsWith('xomtruyen-admin/') || file.startsWith('xomtruyen-admin\\'))
            subsystems.add('Admin Portal');
        else if (file.startsWith('xomtruyen-app/') || file.startsWith('xomtruyen-app\\'))
            subsystems.add('Mobile App');
        else if (file.startsWith('xomtruyen.API/') || file.startsWith('xomtruyen.API\\'))
            subsystems.add('Backend API');
        else if (file.startsWith('xomtruyen-overview/') || file.startsWith('xomtruyen-overview\\'))
...
function buildTechStack() {
    ...
    // 2. Admin Portal (xomtruyen-admin)
    const adminPkg = path.join(WORKSPACE_ROOT, 'xomtruyen-admin', 'package.json');
    if (fs.existsSync(adminPkg)) {
        const result = scanPackageJson(adminPkg, 'xomtruyen-admin (Admin Portal)');
        if (result) projects.push(result);
    }

    // 3. Mobile App (xomtruyen-app)
    const appPkg = path.join(WORKSPACE_ROOT, 'xomtruyen-app', 'package.json');
    if (fs.existsSync(appPkg)) {
        const result = scanPackageJson(appPkg, 'xomtruyen-app (Mobile App)');
        if (result) projects.push(result);
    }

    // 4. Backend API (.NET) - scan .csproj
    const apiCsproj = path.join(WORKSPACE_ROOT, 'xomtruyen.API', 'xomtruyen.API.csproj');
    if (fs.existsSync(apiCsproj)) {
        const result = scanCsproj(apiCsproj, 'xomtruyen.API (Backend API)');
        if (result) projects.push(result);
    }

    // 5. Overview Dashboard
    const overviewPkg = path.join(WORKSPACE_ROOT, 'xomtruyen-overview', 'package.json');
    if (fs.existsSync(overviewPkg)) {
        const result = scanPackageJson(overviewPkg, 'xomtruyen-overview (Dashboard)');
        if (result) projects.push(result);
    }
    ...
    const adminSrc = path.join(WORKSPACE_ROOT, 'xomtruyen-admin', 'src');
    if (fs.existsSync(adminSrc)) {
        importScans.push(scanImports(adminSrc, 'xomtruyen-admin (Admin Portal)'));
    }

    const appSrc = path.join(WORKSPACE_ROOT, 'xomtruyen-app', 'src');
    if (fs.existsSync(appSrc)) {
        importScans.push(scanImports(appSrc, 'xomtruyen-app (Mobile App)'));
    }
...
function main() {
    ...
    console.log('');
    console.log('════════════════════════════════════════════════════════');
    console.log(`✅ HOÀN TẤT! Đã tạo 3 file JSON trong xomtruyen-overview/`);
