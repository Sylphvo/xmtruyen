...
namespace Xmtruyen.API.Services.Background
{
                        ...
                        if (result.Status == "COMPLETED" && Guid.TryParse(workItem.PublicationId, out var pubGuid))
                        {
                            ...
                            if (publication != null)
                            {
                                ...
                                if (result.Output?.TotalPage > 0 && workItem.FileName != null)
                                {
                                    ...
                                    var isLocked = publication.AccessLevel == Xmtruyen.API.Models.Enums.AccessLevel.Vip;
                                    var chapter = new Xmtruyen.API.Models.ComicChapter
                                    {
                                        Id = chapterId,
                                    ...
                                    var pages = new List<Xmtruyen.API.Models.ComicPage>();
                                    for (int i = 0; i < result.Output.TotalPage; i++)
                                    {
                                        pages.Add(new Xmtruyen.API.Models.ComicPage
                                        {
                                            Id = Guid.NewGuid(),
