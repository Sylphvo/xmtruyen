...
import { MOCK_CATEGORIES, STATUSES } from '../constants/mockData';
...
export const Categories: React.FC = () => {
  const [data, setData] = useState<ICategory[]>(MOCK_CATEGORIES);
  ...
  const handleStatusChange = (id: string, newStatus: string) => {
    setData(prev => prev.map(category => category.id === id ? { ...category, status: newStatus } : category));
  };
  ...
  return (
    <div className="card border-0 shadow-sm h-auto" style={{ backgroundColor: 'var(--bs-body-bg)' }}>
      <div className="card-header border-bottom-0 pt-4 pb-3 d-flex justify-content-between align-items-center" style={{ backgroundColor: 'transparent' }}>
        <h5 className="mb-0 fw-semibold" style={{ color: 'var(--bs-heading-color)' }}>Quản lý Category</h5>
            ...
            <tbody>
              ...
              {paginatedData.length > 0 ? (
                paginatedData.map((category) => (
                  <tr key={category.id} style={{ borderBottom: '1px solid var(--bs-border-color)' }}>
                    <td style={{ padding: '12px 16px', backgroundColor: 'transparent' , color: 'var(--bs-body-color)'}}>
                    ...
                    <td style={{ padding: '12px 16px', backgroundColor: 'transparent' , color: 'var(--bs-body-color)'}}>
                      <Dropdown>
                        <Dropdown.Toggle 
                          ...
                          style={{ 
                            ...getStatusStyle(category.status),
                            minWidth: '110px',
                            padding: '6px 12px',
                            boxShadow: 'none'
                          }}
