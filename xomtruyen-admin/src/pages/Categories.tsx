                    ...
                    data.map((category) => (
                      editingId === category.id ? (
                        <tr key={`edit-${category.id}`} className="jira-table-row inline-edit-row" style={{ height: '46px' }}>
                          ...
                          <td style={{ padding: '12px 16px', backgroundColor: 'transparent', color: 'var(--jira-text)' }}>
                            <Form.Control autoFocus size="sm" value={editData.name || ''} onChange={(e) => setEditData({ ...editData, name: e.target.value })} onKeyDown={(e) => handleKeyDown(e, () => handleSaveEdit(category.id), handleCancelEdit)} onBlur={() => handleSaveEdit(category.id)} placeholder="Tên thể loại" className="inline-edit-input text-body w-100" />
                          </td>
                          <td style={{ padding: '12px 16px', backgroundColor: 'transparent', color: 'var(--jira-text)' }}>
