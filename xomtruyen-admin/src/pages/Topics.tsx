...
export const Topics: React.FC = () => {
  ...
  const getCellStyle = (topicId: number, field: string, baseStyle: React.CSSProperties = {}) => {
    const isFocused = focusedCell?.id === topicId && focusedCell?.field === field;
    const isEditing = editingId === topicId && activeEditField === field;
    return {
      ...baseStyle,
  ...
  const [columnOrder, setColumnOrder] = useState<string[]>(() => {
    const savedOrder = localStorage.getItem('topicsColumnOrder');
    if (savedOrder) {
  ...
  useEffect(() => {
    localStorage.setItem('topicsColumnOrder', JSON.stringify(columnOrder));
  }, [columnOrder]);
  ...
  const handleCellKeyDown = (e: React.KeyboardEvent, field: string, topic: ITopic) => {
    if (e.key === 'Enter') {
      e.preventDefault();
  ...
  const handleCellDoubleClick = (topic: ITopic, field: keyof ITopic) => {
                ...
                {columnOrder.map((colId) => {
                  ...
                  switch(colId) {
                    ...
                    case 'name':
                      ...
                      headerContent = <span className="fw-semibold text-nowrap">Tên Chủ đề {getSortIcon('name')}</span>;
                      break;
                    case 'slug':
              ...
              <>
                        ...
                        case 'name':
                          return (
                            <td key={colId} style={{ padding: '12px 16px', backgroundColor: 'transparent', color: 'var(--jira-text)' }}>
                              <Form.Control autoFocus size="sm" value={newItem.name || ''} onChange={(e) => setNewItem({ ...newItem, name: e.target.value })} onKeyDown={(e) => { if (e.key === 'Enter') handleAddSubmit(); else if (e.key === 'Escape') handleCloseAdd(); }} placeholder="Tên chủ đề" className="inline-edit-input text-body w-100" />
                            </td>
                          );
                        case 'slug':
                ...
                {sortedData.length > 0 ? (
                  sortedData.map((topic) => (
                    <tr key={topic.id} className="jira-table-row" style={{ height: '46px', backgroundColor: selectedIds.includes(topic.id) ? '#ebf2fc' : 'transparent' }}>
                      {columnOrder.map((colId) => {
                        switch(colId) {
                          case 'checkbox':
                            return (
                              <td className="jira-sticky-left" key={colId} style={{ borderLeft: 0, padding: '5px 6px', backgroundColor: selectedIds.includes(topic.id) ? 'var(--jira-selected-bg, #ebf2fc)' : 'var(--jira-table-bg, #ffffff)', textAlign: 'center', position: 'sticky', left: 0, zIndex: 2, boxShadow: 'inset -2px 0 4px -2px rgba(0,0,0,0.12)' }} onClick={(e) => e.stopPropagation()}>
                                <Form.Check
                                  type="checkbox"
                          ...
                          case 'name':
                            return (
                              <td key={colId}
                                onClick={() => setFocusedCell({ id: topic.id, field: 'name' })}
                                style={getCellStyle(topic.id, 'name', { padding: editingId === topic.id && activeEditField === 'name' ? '0 16px' : '5px 6px', backgroundColor: 'transparent', color: 'var(--jira-text)' })}
                              >
                                <div className="d-flex align-items-center gap-3 h-100">
                                  {editingId === topic.id && activeEditField === 'name' ? (
                                    <Form.Control
                                      value={editData.name || ''}
                                      onChange={(e) => setEditData({ ...editData, name: e.target.value })}
                                      onKeyDown={(e) => handleCellKeyDown(e, 'name', topic)} 
                                    ...
                                    <span
                                      onDoubleClick={() => handleCellDoubleClick(topic, 'name')}
                                      className="fw-medium flex-grow-1 text-truncate"
                                      style={{ cursor: 'text', color: 'var(--jira-text)' }}
                                      title={editingId === topic.id ? editData.name : topic.name}
                                    >
                                      {editingId === topic.id ? editData.name : topic.name}
                                    </span>
                                  )}
                                </div>
                              </td>
                          ...
                          case 'slug':
                            return (
                              <td key={colId}
                                onClick={() => setFocusedCell({ id: topic.id, field: 'slug' })}
                                style={getCellStyle(topic.id, 'slug', { padding: editingId === topic.id && activeEditField === 'slug' ? '0 16px' : '5px 6px', backgroundColor: 'transparent', color: 'var(--jira-text)' })}
                              >
                                <div className="d-flex align-items-center gap-3 h-100">
                                  {editingId === topic.id && activeEditField === 'slug' ? (
                                    <Form.Control
                                      value={editData.slug || ''}
                                      onChange={(e) => setEditData({ ...editData, slug: e.target.value })}
                                      onKeyDown={(e) => handleCellKeyDown(e, 'slug', topic)} 
                                      onBlur={() => handleSaveEdit(topic.id)}
                                    ...
                                    <span
                                      onDoubleClick={() => handleCellDoubleClick(topic, 'slug')}
                                      className="text-muted small flex-grow-1 text-truncate"
                                      style={{ cursor: 'text', color: 'var(--jira-text)' }}
                                      title={editingId === topic.id ? editData.slug : topic.slug}
                                    >
                                      {editingId === topic.id ? editData.slug : topic.slug}
                                    </span>
                                  )}
                                </div>
                              </td>
                          ...
                          case 'action':
                            return (
                              <td className="jira-sticky-right" key={colId} style={{ borderRight: 0, padding: '5px 6px', backgroundColor: selectedIds.includes(topic.id) ? 'var(--jira-selected-bg, #ebf2fc)' : 'var(--jira-table-bg, #ffffff)', textAlign: 'center', color: 'var(--jira-text)', position: 'sticky', right: 0, zIndex: 2, boxShadow: 'inset 2px 0 4px -2px rgba(0,0,0,0.12)' }}>
                                <Dropdown className="d-inline-block">
                                  ...
                                  <Dropdown.Menu className="shadow-sm border-0 py-2" popperConfig={{ strategy: 'fixed' }} renderOnMount>
                                    <Dropdown.Item onClick={() => handleDelete(topic.id)} className="py-2 px-3 text-danger" style={{ fontSize: '14px' }}>
                                      Xóa
                                    </Dropdown.Item>
                                  </Dropdown.Menu>
                                </Dropdown>
