...
export const Users: React.FC = () => {
  ...
  const [selectedUserForRole, setSelectedUserForRole] = useState<{ id: string | string[], name: string } | null>(null);
  const [bulkPopoverAction, setBulkPopoverAction] = useState<'status' | null>(null);
  ...
  const handleAddSubmit = async (e?: React.FormEvent | React.KeyboardEvent) => {
    ...
    if (!finalEmail.includes('@')) {
      finalEmail += '@xmtruyen.vn';
    }

    const payload = { ...newUser, email: finalEmail };
    ...
    try {
      await Promise.all(
        selectedUserIds.map(async (id) => {
          ...
          updateItem(id, (prev) => ({ ...prev, isActive }) as any);
        })
      );
      toast.success('Đã cập nhật trạng thái các user được chọn!');
      setSelectedUserIds([]);
    } catch (error) {
    ...
    <>
      {bulkPopoverAction && (
        <FloatingBulkActionPopover
          title={bulkPopoverAction === 'status' ? 'Trạng thái' : ''}
          options={
      ...
      <div className="jira-table-container" style={{ position: 'relative', opacity: (roleModalVisible || !!bulkPopoverAction) ? 0.5 : 1, transition: 'opacity 0.15s ease', pointerEvents: (roleModalVisible || !!bulkPopoverAction) ? 'none' : undefined }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', border: '2px solid #4c9aff', borderRadius: '3px', backgroundColor: '#fff', padding: '4px 8px', boxShadow: '0 1px 2px rgba(0,0,0,0.05)' }}>
              <div style={{ flex: 1, display: 'flex', alignItems: 'center' }}>
                ...
                <input
                  ...
                  placeholder="Nhập email người dùng mới (ví dụ: user@xmtruyen.com)..."
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <Button 
                  variant="light" 
                  size="sm" 
                  style={{ backgroundColor: '#0052cc', color: '#fff', border: 'none', borderRadius: '3px', fontWeight: 500, padding: '4px 12px', opacity: !newUser.email ? 0.6 : 1 }}
                  onClick={(e) => handleAddSubmit(e)}
                  disabled={isSubmitting || !newUser.email}
                >
      ...
      <FloatingBulkActionBar 
        ...
        onBulkEdit={() => {
          setSelectedUserForRole({ id: selectedUserIds, name: `${selectedUserIds.length} người dùng đã chọn` });
